...
class ImageCompressor(Tk):
    def __init__(self, winTitle, xSize, ySize, *args):
        ...
        self.compressFile = Button(text="[BG]Choose Image", command=self.GetImageFile)
        self.compressFile.place(x=25, y=15)
        self.chooseQuality = Label(self, text="[BG]Choose Image quality", font=("Courier", 10))
        self.chooseQuality.place(x=60, y=70)
        self.scaleValue = Scale(self, from_=100, to=0)
        self.scaleValue.place(x=0, y=70)
        self.saveFolder = Button(text="[BG]Choose which folder to save to", command=self.SavedFolder)
        self.saveFolder.place(x=62.5, y=100)
        self.imageNameLabel = Label(text="[BG]Enter new file name")
        self.imageNameLabel.place(x=62.5, y=135)
        self.imageName = Entry(self, bd=3)
        self.imageName.place(x=62.5, y=160)
        self.compressImageBtn = Button(text="[BG]Compress Image", command=self.CompressImage, bd=5)
    ...
    def SavedFolder(self):
        ...
        if self.saveTo:
            messagebox.showinfo("BlackGuard : Save to:", self.saveTo)
        else:
            messagebox.showwarning("Error", "BlackGuard : No folder selected")
    ...
    def CompressImage(self):
        ...
        try:
            ...
            messagebox.showinfo("BlackGuard : Successful", f"BlackGuard : Compressed image saved to {self.saveTo}")
        except:
            messagebox.showwarning("Error", "BlackGuard : Something went wrong")

MyNewGUI = ImageCompressor("Image Compressor | Creator : MR.D3F417", 450, 225)